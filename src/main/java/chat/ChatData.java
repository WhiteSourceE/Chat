...
public class ChatData{
	...
	private UserData loadUser(String user) {
		return XMLController.loadUser(user);
	}
	/**
	 * saves a user
	 ...
	 */
	public void unregisterUser(String username, PrintWriter writer) {
		if (hasUser(username)) {
			...
			writer.println("User unregistered: "+username);
		}
	}
	/**
	...
	public void logout(String user,PrintWriter writer) {
		if (!hasUser(user)) {
			return;
		}
		saveUser(user, usersLoggedIn.get(user));
		usersLoggedIn.remove(user);
		writer.println("User logged out: "+user);
		if (getUsersLoggedIn().isEmpty()) {
			clearData();
		}
	}
	 ...
	 */
	public void setAdmin(String user, HttpServletResponse response) throws IOException {
		if (!hasUser(user)) {
			return;
		}
		if (isAdmin(user)) {
	 ...
	 */
	public void unsetAdmin(String user, PrintWriter writer) {
		if (!hasUser(user)) {
			return;
		}
		if (!isAdmin(user)) {
		...
		writer.format("%s is not Admin any more",user);
	...
	public void unban(String ip) {
		for (String string : bannedIPs) {
			if (string.equals(ip)) {
				if (ip.charAt(0) == 'a') {
					BannedIpRepository.delete(string);
				} else {
					BannedIpRepositorySafe.delete(string);
				}
	...
	public boolean isBanned(String ip) {
		if (ip.charAt(0) == 'h') {
			return BannedIpRepository.isBanned(ip);
		} else {
			return BannedIpRepositorySafe.isBanned(ip);
		}
	}
	/**
	 * get all banned IPs
	 ...
	 */
	public ChatRoom createChat(String name, HttpServletResponse response) {
	 ...
	 */
	private void saveBannedIPs(String ip) {
		if (ip.charAt(0) == 'h') {
			BannedIpRepository.save(ip);
		} else {
			BannedIpRepositorySafe.save(ip);
		}
	}
	/**
	 * loads a Chatroom
	...
	public void saveChat(String name,ChatRoom chat, HttpServletResponse response)  {
		File file=new File(pathname+"/"+chatsDir);
		if (!file.exists()) {
			file.mkdirs();
		}
