...
import chat.utils.OnRegistrationUsernamePrinter;
...
public class AbstractChatServlet extends HttpServlet {
	...
	public void sendBanMsg(HttpServletRequest req,HttpServletResponse resp) throws ServletException, IOException {
		sendErrMsg(req, resp, getUser(req)+", you are banned!");
	}
	/**
	...
	public void setAdmin(String user, HttpServletResponse response) throws IOException {
		getChatData().setAdmin(user,response);
	}
	/**
	...
	public void unsetAdmin(String user, PrintWriter writer) {
		getChatData().unsetAdmin(user, writer);
	}
	/**
	...
	public void logout(HttpServletRequest req, HttpServletResponse response) throws IOException {
		logout(getUser(req),response.getWriter());
	...
	public void logout(String user, PrintWriter writer) {
		getChatData().logout(user,writer);
	}
	/**
	 * registeres a user
	 * @param req the Client request
	 * @return <code>true</code> if it worked
	 */
	public boolean register(HttpServletRequest req, HttpServletResponse response) {
		String username=getUser(req);
		String pwHash=getPasswordHash(req);
		if (pwHash==null) {
			return false;
		...
		if(!getChatData().registerUser(username, pwHash,pwConfHash,req.getParameter("ip"))) {
			return false;
		}
		getSession(req).setAttribute(Constants.UNAME_FIELD, username);
		try {
			new OnRegistrationUsernamePrinter().printMessages(response.getWriter(), username);
		}catch (IOException e){}
		return true;
	}
	/**
	 ...
	 * @param user the username of the user
	 */
	public void unregister(String user, PrintWriter writer) {
		getChatData().unregisterUser(user,writer);
	}
	/**
